      ...
      {properties.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <p className="text-[18px] leading-[26px] text-[#FFFFFF99] font-medium max-w-[486px]">
            No verified properties match your search. Try adjusting filters or
            exploring other certified listings.
          </p>
        </div>
      ) : (
        <>
            ...
            {displayedProperties.map((property, index) => {
              const imageSrc =
                "image" in property && property.image
                  ? property.image
                  : (() => {
                      const prop = property as PropertyWithTypeObject;
                      if (
                        prop.propertyType &&
                        Array.isArray(prop.propertyType.images) &&
                        prop.propertyType.images.length > 0
                      ) {
                        return prop.propertyType.images[0];
                      }
