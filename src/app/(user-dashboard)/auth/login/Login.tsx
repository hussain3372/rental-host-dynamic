...
export default function LoginPage() {
  ...
  const googleLogin = async () => {
    try {
      ...
      // Redirect to Google OAuth with callback to redirecting page
      await signIn("google", {
  ...
  const handleLogin = async (formData: LoginFormData) => {
    try {
      ...
      if (mfaRequired === true) {
        ...
        const email = (response?.data as MfaResponse)?.email || formData.email;
      ...
      if (user) {
        if (user.firstname)
          localStorage.setItem("hostFirstname", user.firstname);
