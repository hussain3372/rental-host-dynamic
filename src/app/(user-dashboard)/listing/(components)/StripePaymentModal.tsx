  ...
  const CheckoutForm: React.FC<{
    ...
    onClose: () => void;
    amount: number;
    applicationId?: string; // Add this prop
  }> = ({ onSuccess, onClose, amount, applicationId }) => {
    ...
    const handleSubmit = async (e: React.FormEvent): Promise<void> => {
      ...
      try {
        // 1️⃣ Get application ID (use prop if provided, otherwise fallback to localStorage)
        let appId: string | undefined = applicationId;
        ...
        const response = await application.createPayment({
          ...
          applicationId: appId, // Use the appId variable
        },
        appId
      );
          ...
          <div className="p-6">
            <Elements stripe={stripePromise}>
              <CheckoutForm
                ...
                onClose={onClose}
                amount={amount}
                applicationId={applicationId} // Pass the prop here
              />
            </Elements>
          </div>
