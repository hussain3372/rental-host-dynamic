  ...
  interface ApplicationData {
    ...
    payments?: PaymentDetails[]; // Add this line
    currentStep?: string; // Add this if needed
  }
  ...
  const fetchApplicationData = async (): Promise<ApplicationData | null> => {
    try {
      ...
      const storedData: ApplicationData | null = stored ? JSON.parse(stored) : null;
      
      console.log("🔍 fetchApplicationData() called");

      if (!storedData?.id) {
        console.warn("No application ID found in local storage");
        return null;
      }
      ...
      if (response.success && response.data) {
        const applicationData: ApplicationData = response.data as ApplicationData;
      ...
      console.warn("Failed to fetch application data:", response.message);
      return null;
    } catch (error: unknown) {
      console.error("Error fetching application data:", error);
      const errorMessage: string = error instanceof Error ? error.message : "Unknown error";
      toast.error(`Failed to fetch application data: ${errorMessage}`);
      return null;
    }
  };
  ...
  try {
    ...
    // If uploadDocuments returns empty array, it means upload failed
    ...
    if (success) {
      ...
      toast.error("Failed to update application step", { id: toastId });
      return false;
    }
  } catch (error) {
  ...
  const handlePayment = async (): Promise<boolean> => {
    ...
    const toastId: string = toast.loading("Verifying payment status...");

    try {
      // Fetch fresh application data to check payment status
      const freshAppData: ApplicationData | null = await fetchApplicationData();
      
      if (!freshAppData?.id) {
        toast.error("No application found. Please complete previous steps first.", { id: toastId });
        return false;
      }

      console.log("🔍 Checking payment status from API:", {
        applicationId: freshAppData.id,
        payments: freshAppData.payments
      });

      // Check if payment exists and is completed
      if (freshAppData.payments && freshAppData.payments.length > 0) {
        const hasCompletedPayment: boolean = freshAppData.payments.some((payment: PaymentDetails) => 
        ...
        if (hasCompletedPayment) {
          console.log("✅ Payment verified from API");
          // Update local storage with fresh data
          localStorage.setItem("applicationData", JSON.stringify(freshAppData));
          toast.success("Payment verified! You can now proceed.", { id: toastId });
          return true;
        } else {
          console.log("❌ Payment exists but not completed:", freshAppData.payments);
          // const paymentStatus: string = freshAppData.payments[0]?.status || 'unknown';
          // toast.error(`Payment status: ${paymentStatus}. Please complete the payment process.`, { id: toastId });
          return false;
        }
      } else {
        console.log("❌ No payments found in application");
        toast.error("No payment found. Please complete the Stripe payment first.", { id: toastId });
        return false;
      }
      ...
      const errorMessage: string = error instanceof Error ? error.message : "Unknown error";
      toast.error(`Error verifying payment: ${errorMessage}`, { id: toastId });
      return false;
    }
  };
    ...
    try {
      ...
      if (step === 1 && subStep === 1) {
        ...
        if (success) {
          const stored: string | null = localStorage.getItem("applicationData");
          if (stored) {
            const appData: ApplicationData = JSON.parse(stored);
            if (!appData.documents || appData.documents.length < 4) {
              console.error("❌ Documents validation failed");
              toast.error("Documents were not saved properly. Please upload all documents again.");
              success = false;
            }
          }
        }
        ...
        if (!success) {
          console.error("Document upload failed - preventing step progression");
          setIsLoading(false);
          return;
        }
      } else if (step === 4) {
        ...
        if (!success) {
          console.error("❌ Payment verification failed - preventing step progression");
          setIsLoading(false);
          return;
        }
        
        console.log("✅ Payment verified, moving to step 5");
        // Payment is verified, move to step 5
        setStep(5);
        setIsLoading(false);
        return;
      } else if (step === 5) {
      ...
      if (success) {
        ...
        console.error(`Step ${step} processing failed - preventing progression`);
  ...
  const renderStepContent = () => {
    ...
    // ✅ Check if payment is required before showing Step 4
    if (step === 4) {
      const stored = localStorage.getItem("applicationData");
      const localApplicationData = stored ? JSON.parse(stored) : null;
      
      // If payment already exists, skip to step 5
      if (localApplicationData?.payments && localApplicationData.payments.length > 0) {
        setStep(5);
        return <Step5 />;
      }
    ...
    if (step === 1 && subStep === 1) {
      ...
      // ✅ Check if we should skip step 4 if no payment is needed
      const stored = localStorage.getItem("applicationData");
      const localApplicationData = stored ? JSON.parse(stored) : null;
      
      if (step === 3 && localApplicationData?.payments && localApplicationData.payments.length > 0) {
        // Skip directly to step 5 if payment already exists
        setStep(5);
      } else {
