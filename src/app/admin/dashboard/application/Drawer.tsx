...
export default function TicketDrawer({ 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isRejection && applicationId) {
        // Call the API with reviewNotes as third parameter
        const result = await applicationApi.approveORrejectApplication(
          applicationId,
          "reject",
          subject.trim() // This will be sent as reviewNotes in the payload
        );
      ...
      >
        <div className="space-y-5">
          <h2 className="text-[16px] sm:text-[20px] leading-6 font-medium mb-3">
            {isRejection ? 'Reject Application' : 'Add a Note'}
          </h2>
          <p className="text-[12px] sm:text-[16px] sm:leading-5 font-normal mb-10 text-[#FFFFFF99]">
            {isRejection 
              ? 'Please provide a reason for rejecting this application. This note will be sent to the applicant as a review note.'
          ...
          <div>
            <label className="block text-[14px] leading-[18px] font-medium mb-[10px]">
              {isRejection ? 'Rejection Reason' : 'Note'}
            </label>
            <textarea
              placeholder={
                isRejection 
                  ? "Explain why this application is being rejected..."
                  : "Write down short description for the host..."
              }
              value={subject}
              rows={4}
        ...
        <div className="mt-6">
          ...
          >
            {isSubmitting ? 'Submitting...' : (isRejection ? 'Reject Application' : 'Submit Note')}
          </button>
        </div>
      </div>
    </div>
