...
import type { Application, Document } from "@/app/api/super-admin/user-management/types";
...
export default function Checklist({ notes, application }: ChecklistProps) {
  ...
  const handleApproveReject = async (action: 'approve' | 'reject') => {
    ...
    try {
      setIsLoading(true);
      const response = await managementApi.approveORrejectApplication(application.id, action);
    ...
    <div className='pb-5 pt-[60px]'>
      ...
      {documents.length > 0 && (
        <div className="pt-[60px]">
          <h3 className='font-semibold text-[16px] leading-[20px] tracking-normal pb-5'>Uploaded Documents</h3>
          <div className='w-full'>
            <div className='rounded-lg w-full grid grid-cols-1 sm:grid-cols-2 gap-3'>
              {documents.map((document) => (
                <div key={document.id} className='flex p-3 bg-[#121315] rounded-lg w-full items-center gap-5'>
                  <Image 
                  ...
                  <div>
                    <h3 className='font-medium text-[12px] sm:text-[18px] leading-[16px] sm:leading-[22px] text-white xl:w-[353px]'>
                      {getDocumentTypeDisplayName(document.documentType)} Document
                    </h3>
