...
import type { Application } from "@/app/api/super-admin/user-management/types";
...
export default function ApplicationDetail() {
  ...
  useEffect(() => {
    const fetchApplicationDetail = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await managementApi.getApplicationDetail(
          id as string
        );
    ...
    const fetchPropertyTypeName = async (propertyTypeId: string) => {
      try {
        ...
        const response = await managementApi.getPropertyType(propertyTypeId);
